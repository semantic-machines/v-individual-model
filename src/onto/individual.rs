...
use crate::onto::parser::{parse_to_predicate, RawType};
...
pub struct RawObj {
    pub data: Vec<u8>,
    pub(crate) cur: u64,
    pub(crate) len_predicates: u32,
    pub(crate) cur_predicates: u32,
...
impl Individual {
    ...
    pub fn new_from_obj(obj: &IndividualObj) -> Self {
        let mut new_obj = IndividualObj {
            uri: obj.uri.to_owned(),
    ...
    pub fn compare(&self, b: &Individual, ignore_predicates: Vec<&str>) -> bool {
        ...
        let mut counter_b = 0;
        for predicate in b.obj.resources.keys() {
            if ignore_predicates.contains(&predicate.as_str()) {
                continue;
            }
            counter_b += 1;
